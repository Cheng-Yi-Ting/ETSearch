...
def getRecommendations(prefs, person, similarity=compute_cosine_similarity):
    ...
    for item, total in totals.items():
        # print(totals[item])
        # print(simSums[item])
        rating=user_avgScore+(totals[item]/simSums[item])
        # if(rating>3.5):
...
def topMatches(prefs, person, n=5, similarity=compute_cosine_similarity):
    ...
    for i in scores[0:n]:
        user=i[0]
        sim_critics[user]=all_critics[user]
        # print(i[0])
    sim_critics[person]=all_critics[person]
    # print(sim_critics)
    # all_critics[user['name']]=user_critics 
    # print(sim_critics)
    # return scores[0:n]
...
def main(argv):
    ...
    for person in all_critics:
        ...
        user_avg=round(sum_score/cnt,2)
        if username==person:
            user_avgScore=user_avg

        for item in all_critics[person]:
            all_critics[person][item]=all_critics[person][item]-user_avg
    # print(username)
    # remove user seen news
    # print(all_critics)
    get_users_view(username)
    topMatches(all_critics,username)
    rec_result=getRecommendations(sim_critics,username)
    rec_news_list=[]

    for i in rec_result:
        rec_news_list.append(i[1])
    # print(rec_news_list)
    rec_list=rec_news_list[0:6]
